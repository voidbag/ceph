...
OSD::PG_map_subscriber::PG_map_subscriber(OSD *o) {
  ...
  rcu_read_lock();
  subscribed_pg_map = rcu_dereference(o->pg_map_ptr);
}
...
OSD::WLocker_and_pg_map_publish::WLocker_and_pg_map_publish(OSD *o) :
  ...
  rcu_xchg_pointer(&osd_ptr->pg_map_ptr, &osd_ptr->pg_map_shadow);
  synchronize_rcu();
}

OSD::WLocker_and_pg_map_publish::~WLocker_and_pg_map_publish() {
  rcu_xchg_pointer(&osd_ptr->pg_map_ptr, &osd_ptr->pg_map);
  synchronize_rcu();
}
